...
>[!IMPORTANT] El siguiente documento sólo se aplica a IAB 2.0. Los usuarios necesitan utilizar Visitante.js versión 5.0 para trabajar con IAB 2.0.

Conecte la Plataforma de gestión de consentimiento (CMP) con el complemento de transparencia y marco de consentimiento (TCF) de IAB de inclusión.

Los clientes de Adobe Audiencia Manager que utilizan [IAB TCF](https://iabtechlab.com/standards/gdpr-transparency-and-consent-framework/) pueden conectar su plataforma de gestión de consentimiento (CMP) con el complemento TCF de IAB de inclusión. El servicio de inclusión (Opt-in) es una función integrada en la biblioteca JavaScript de ECID que puede deshabilitar bibliotecas determinadas de soluciones de Adobe, en función de las preferencias del visitante, que se establecen dentro de una plataforma CMP. Cuando se implementa el complemento TCF IAB de Opt-in con la biblioteca ECID, las preferencias de visitante de su CMP que admite TCF IAB se asignan automáticamente a Opt-in. Estas preferencias activarán bibliotecas basadas en Audience Manager (DIL y ECID) y llamadas asociadas cuando se reciba el consentimiento.
...
Utilice Adobe Experience Platform Launch para implementar el complemento TCF de IAB para su sitio. Al habilitar IAB para la opción de inclusión de forma manual, asegúrese de que la siguiente configuración se establezca en true en el objeto Visitante:
