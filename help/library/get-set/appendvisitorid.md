---
description: Esta función le permite compartir un ECID de un visitante entre dominios cuando los navegadores bloquean las cookies de terceros. Para utilizar esta función, debe haber implementado el servicio de ID de visitante y ser el propietario de los dominios de origen y destino. Disponible en VisitorAPI.js versión 1.7.0 o posteriores.
...
>[!TIP]
>
>El seguimiento entre dominios no funcionará como se pretende si el ECID se rechaza inicialmente (o anteriormente). No comprobará los ID existentes que se hayan pasado a través de una URL o que existían anteriormente en la cookie, teniendo en cuenta que eran los ID cuando el consentimiento se estableció en &quot;NO&quot;.

Esta función le permite compartir un ECID de un visitante entre dominios cuando los navegadores bloquean las cookies de terceros. Para utilizar esta función, debe haber implementado el servicio de ID de visitante y ser el propietario de los dominios de origen y destino. Disponible en `VisitorAPI.js` versión 1.7.0 o superior.
...
El servicio de identificación del visitante escribe una cookie propia y de terceros en el explorador cuando una persona visita su sitio (consulte [Cookies y el servicio de identificación del visitante](../../introduction/cookies.md) ). La cookie de origen contiene el MID, un ID único para ese visitante. La cookie de terceros contiene otro ID utilizado por el servicio de ID de visitante para generar el MID. Cuando un explorador bloquea esta cookie de terceros, el servicio de ID de visitante no puede:
...
Para ayudar a resolver este problema, implemente una `Visitor.appendVisitorIDsTo( *`URL`*)`. Esta propiedad permite que el servicio de ID de visitante rastree los visitantes del sitio en varios dominios, incluso cuando sus exploradores bloquean las cookies de terceros. Funciona de esta forma:
...
* El código del servicio de ID de visitante en el dominio de destino extrae el MID de la URL, en lugar de enviar una solicitud a Adobe para obtener el ID de ese visitante. Esta solicitud incluye el ID de la cookie de terceros, que no está disponible en este caso.
* El código del servicio de ID de visitante en la página de destino emplea el MID transferido para hacer un seguimiento del visitante.
  ...
  <tr> 
   ...
   <td colname="col2"> 
    <ul id="ul_9D7933FF68EE4C71BAE999B3747F8398"> 
     <li id="li_9036C76AAECC4E639C23020C0C9F2AF8"> <a href="https://experienceleague.adobe.com/docs/mobile-services/android/experience-cloud-android/mc-methods.html?lang=es" format="https" scope="external"> Android Visitor ID Service Methods </a> </li> 
     <li id="li_E49D357905584674BFDFE348345B3849"> <a href="https://experienceleague.adobe.com/docs/mobile-services/ios/exp-cloud-ios/mc-methods.html?lang=es" format="https" scope="external"> iOS Visitor ID Service Methods </a> </li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table> 
